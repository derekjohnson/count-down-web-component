class Countdown extends HTMLElement {
  ...
  render() {
    const { shadowRoot, ends, breakpoint1, breakpoint2 } = this;
  ...
  initialiseClock(endDate) {
    const { shadowRoot, createCountdownItem, getTimeRemaining } = this;
    ...
    timeUnits.forEach((timeUnit) => {
      if(timeUnit !== 'total') {
        clock.appendChild(createCountdownItem(timeUnit, timeRemaining[timeUnit]));
      }
    });
    
    function updateClock() {
      const endValue = document.querySelector('count-down').getAttribute('ends');
      const timeRemaining = getTimeRemaining(endValue);
      ...
      clock.setAttribute('datetime', `P${timeRemaining.days}DT${timeRemaining.hours}H${timeRemaining.minutes}M${timeRemaining.seconds}S`);
